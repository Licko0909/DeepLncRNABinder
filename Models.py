...
def smallRNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,HIDDEN_UNITS,name): 
    #HIDDEN_UNITS = 5
    model = Sequential()
    model.add(Bidirectional(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True,dropout=0.3),input_shape =x_train.shape[1:],merge_mode='concat'))
    ...
    filepath=name+str(HIDDEN_UNITS)+"best_smallRNN.hdf5"
...
def mediumRNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,HIDDEN_UNITS,name):
    #HIDDEN_UNITS = 15
    model = Sequential()
    if(DROP_OUT == 1):
        model.add(Bidirectional(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True,dropout=0.3),input_shape =x_train.shape[1:]))
        model.add(MaxPooling1D(pool_size=10,strides=5))
    else:
        model.add(Bidirectional(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True),input_shape=x_train.shape[1:]))
        model.add(MaxPooling1D(pool_size=10,strides=5))
    model.add(Flatten())
    model.add(Dense(1))
    #a soft max classifier
    ...
    filepath=name+"best_mediumRNN_dropout_"+str(DROP_OUT)+".hdf5"
    ...
    print('\rroc-auc: %s - roc-auc_val: %s' % (str(round(roc_train,4)),str(round(roc_test,4))),end=100*' '+'\n')
...
def largeRNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,name):
    HIDDEN_UNITS = 20
    model = Sequential()
    if(DROP_OUT == 1):
        model.add(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True,dropout=0.3,input_shape =x_train.shape[1:]))
        model.add(MaxPooling1D(pool_size=10,strides=5))
        model.add(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True,dropout=0.3))
    else:
        model.add(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True,input_shape=x_train.shape[1:]))
        model.add(MaxPooling1D(pool_size=10,strides=5))
        model.add(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True))
    ...
    filepath=name+"best_largeRNN_dropout_"+str(DROP_OUT)+".hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='max')
    early_stopping_monitor = EarlyStopping(monitor='val_loss',patience = 8)
    #callbacks_list = [checkpoint]
    model.compile(loss=LOSS, optimizer = 'Adam', metrics =METRICS)
...
def smallCNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,KERNEL_SIZE,NUM_KERNEL,name):
    x_val = x_test[range(0,x_test.shape[0],2),:]
    y_val = y_test[range(0,y_test.shape[0],2)]
    x_test = x_test[range(1,x_test.shape[0],2),:]
    y_test = y_test[range(1,y_test.shape[0],2)]
    ...
    model.add(MaxPooling1D())
    model.add(Conv1D(NUM_KERNEL,kernel_size=KERNEL_SIZE,kernel_initializer=KERNEL_INITIAL,kernel_constraint = maxnorm(2)))
    model.add(Activation("relu"))
    model.add(BatchNormalization())
    model.add(Dropout(0.5))
    ...
    filepath=name+"best_smallCNN_dropout_"+str(NUM_KERNEL)+".hdf5"
...
def mediumCNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,KERNEL_SIZE,NUM_KERNEL,name):
    x_val = x_test[range(0,x_test.shape[0],2),:]
    y_val = y_test[range(0,y_test.shape[0],2)]
    x_test = x_test[range(1,x_test.shape[0],2),:]
    y_test = y_test[range(1,y_test.shape[0],2)]
    ...
    model.add(Activation("sigmoid"))
    #filepath="smallCNN_dropout_"+str(DROP_OUT)+".hdf5"
    filepath=name+"best_mediumCNN_dropout_"+str(NUM_KERNEL)+".hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
    early_stopping_monitor = EarlyStopping(monitor='val_loss',patience = 8)
    #callbacks_list = [checkpoint]
    model.compile(loss=LOSS, optimizer = OPTIMIZER, metrics =METRICS)
...
def largeCNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,KERNEL_SIZE,NUM_KERNEL,name):
    x_val = x_test[range(0,x_test.shape[0],2),:]
    y_val = y_test[range(0,y_test.shape[0],2)]
    x_test = x_test[range(1,x_test.shape[0],2),:]
    y_test = y_test[range(1,y_test.shape[0],2)]
    ...
    model.add(Activation("sigmoid"))
    #filepath="smallCNN_dropout_"+str(DROP_OUT)+".hdf5"
    filepath=name+"best_largeCNN_dropout_"+str(NUM_KERNEL)+".hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
    early_stopping_monitor = EarlyStopping(monitor='val_loss',patience = 8)
    #callbacks_list = [checkpoint]
    model.compile(loss=LOSS, optimizer = OPTIMIZER, metrics =METRICS)
...
def verylargeCNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,KERNEL_SIZE,NUM_KERNEL,name):
    x_val = x_test[range(0,x_test.shape[0],2),:]
    y_val = y_test[range(0,y_test.shape[0],2)]
    x_test = x_test[range(1,x_test.shape[0],2),:]
    y_test = y_test[range(1,y_test.shape[0],2)]
...
def smallCNN_RNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,KERNEL_SIZE,name):
    model = Sequential()
    model.add(Conv1D(20,kernel_size=KERNEL_SIZE,kernel_initializer=KERNEL_INITIAL,kernel_constraint = maxnorm(2),input_shape = INPUT_SHAPE))
    model.add(Activation("relu"))
    ...
    if(DROP_OUT == 1):
        ...
        model.add(Bidirectional(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True)))
        model.add(Bidirectional(LSTM(HIDDEN_UNITS,activation='tanh',inner_activation='sigmoid',kernel_constraint = maxnorm(2),kernel_initializer=KERNEL_INITIAL,return_sequences=True)))
    ...
    model.compile(loss=LOSS, optimizer = OPTIMIZER, metrics =METRICS)
    #filepath="largeRNN_dropout_"+str(DROP_OUT)+"_{epoch:02d}_{val_acc:.2f}.hdf5"
    filepath=name+"best_smallCNN_RNN_dropout_"+str(DROP_OUT)+".hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='max')
    early_stopping_monitor = EarlyStopping(monitor='val_loss',patience = 8)
    #callbacks_list = [checkpoint]
    model.compile(loss=LOSS, optimizer = OPTIMIZER , metrics =METRICS)
...
def mediumCNN_RNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,KERNEL_SIZE,name):
    model = Sequential()
    model.add(Conv1D(32,kernel_size=KERNEL_SIZE,kernel_initializer=KERNEL_INITIAL,kernel_constraint = maxnorm(2),input_shape = INPUT_SHAPE))
    model.add(Activation("relu"))
    model.add(MaxPooling1D(pool_size=10,strides=10))
    #model.add(Flatten())
    #model.add(Reshape((50,1))) # shape becomes (batch_size,200,1)
    ...
    model.compile(loss=LOSS, optimizer = OPTIMIZER, metrics =METRICS)
    filepath="largeRNN_dropout_"+str(DROP_OUT)+"_{epoch:02d}_{val_acc:.2f}.hdf5"
    filepath=name+"best_mediumCNN_RNN_dropout_"+str(DROP_OUT)+".hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='max')
    early_stopping_monitor = EarlyStopping(monitor='val_loss',patience = 8)
    #callbacks_list = [checkpoint]
    model.compile(loss=LOSS, optimizer = OPTIMIZER, metrics =METRICS)
...
def largeCNN_RNN(x_train,y_train,x_test,y_test,DROP_OUT,INPUT_SHAPE,KERNEL_SIZE,name):
    model = Sequential()
    model.add(Conv1D(64,kernel_size=KERNEL_SIZE,kernel_initializer=KERNEL_INITIAL,kernel_constraint = maxnorm(2),input_shape = INPUT_SHAPE))
    model.add(Activation("relu"))
    model.add(MaxPooling1D(pool_size=10,strides=5))
    model.add(Conv1D(64,kernel_size=KERNEL_SIZE,kernel_initializer=KERNEL_INITIAL,kernel_constraint = maxnorm(2)))
    model.add(Activation("relu"))
    model.add(MaxPooling1D(pool_size=10,strides=5))
    #model.add(Flatten())
    #model.add(Reshape((50,1))) # shape becomes (batch_size,200,1)
